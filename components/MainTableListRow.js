...
const MainTableListRow = props => {
    ...
    const priceStatus = props.row.increasing ? '-increase' :  '-decrease'
    const prettyfyPrice = (price) => {
    ...
    <div className={`${props.componentClass}__content -price ${priceStatus}`}>{prettyfyPrice(props.row.priceUsd)}</div>
    <div className={`${props.componentClass}__content -market -visibleMd`}>{props.row.marketCapUsd}</div>
    <div className={`${props.componentClass}__content -priceChange -visibleMd`}>{props.row.changePercent24Hr}</div>
    <div className={`${props.componentClass}__content -qty -visibleMd`}>
